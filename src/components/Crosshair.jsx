...
const Crosshair = ({ color = '#FFD700', containerRef = null }) => {
  ...
  useEffect(() => {
    ...
    const startRender = () => {
      const rafLoop = () => {
        const rs = renderedStylesRef.current;
        const m = mouseRef.current;
        rs.tx.current = m.x;
        rs.ty.current = m.y;

        for (const key in rs) {
          rs[key].previous = lerp(rs[key].previous, rs[key].current, rs[key].amt);
        }

        gsap.set(lineVerticalRef.current, { x: rs.tx.previous });
        gsap.set(lineHorizontalRef.current, { y: rs.ty.previous });
    ...
    const onFirstMove = () => {
      const rs = renderedStylesRef.current;
      const m = mouseRef.current;
      rs.tx.previous = rs.tx.current = m.x;
      rs.ty.previous = rs.ty.current = m.y;
